...
public class Boss3 : Boss
{
    ...
    public void EpicAttack()
    {
        ...
        for (int i = 0; i < _col.Length; i++)
        {
            if (_col[i].CompareTag("Player"))
            {
                player.GetComponent<Player>().Hurt(4);
            }
        }
    }
    public void Attack3()
    ...
    public override void Hurt(int Damge)
    {
        if(!invincibility)
        {
            ...
            else if (hp <= MaxHp * 0.4f && !isEpicMode)
            {
                isChanging = true;
                isEpicMode = true;
                invincibility = true;
                animator.SetBool("IsEpicMode", true);
            }
            else if (hp <= MaxHp * 0.1f && isEpicMode)
            {
                rigid2D.velocity = Vector2.zero;
                epicOn = true;
